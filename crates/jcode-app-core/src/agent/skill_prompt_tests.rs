...
async fn active_skill_lives_in_the_dynamic_prompt_and_leaves_no_transcript_residue() {
...

    {
        ...
        controller
            ...
            .expect("workflow state patch");
    }

    let with_state = agent.build_system_prompt_split(None);
    assert!(with_state.dynamic_part.contains("# Workflow Run State"));
    assert!(
        with_state
            .dynamic_part
    ...
    assert!(!with_state.static_part.contains("# Workflow Run State"));
    assert_eq!(
        split.static_part, with_state.static_part,
        "workflow run state must stay out of the cacheable prompt prefix"
    );
