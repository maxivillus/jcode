...
impl Session {
    ...
    /// Сообщает, что содержимое блоков в транскрипте изменилось на месте.
    ///
    /// Число сообщений при такой замене не меняется, поэтому пересборку кэша
    /// провайдерских сообщений нужно запросить явно. Иначе следующий запрос
    /// всё ещё содержал бы заменённые блоки.
    pub fn mark_block_content_changed(&mut self) {
        self.mark_memory_profile_dirty();
        self.mark_messages_full_dirty();
    }
}
